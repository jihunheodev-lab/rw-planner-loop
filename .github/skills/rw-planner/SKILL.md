...
6. **Task Decomposition**: Create 2–6 atomic `TASK-XX-*.md` in `.ai/tasks/` with frontmatter `status: pending`, acceptance criteria, user path, and fast task-scoped verification commands. Prefer independent same-phase tasks to widen DAG parallelism. Put full regression commands in `TASK-00` phase/final gate policy.
